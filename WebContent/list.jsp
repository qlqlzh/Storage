...
<!--
	...
	저장한 후 1페이지 분량의 글을 브라우저에 출력하는 페이지(listView22.jsp)로 넘겨준다.
-->
...
//	이전 페이지에서 넘어오는 currentPage가 없으면 currentPage를 1로 초기화시켜서 사용하고 넘어오는 currentPage가
//	있으면 넘어온 currentPage 값으로 초기화시켜서 사용한다.
	int currentPage = 1;
	try {
...
//	넘어온 검색어가 있으면 카테고리와 검색어를 세션에 저장하고 넘어온 검색어가 없으면 세션에 저장된 카테고리와
//	검색어를 읽어온다.
	if (item != null) {
...
//	글을 입력할 때 엔터키를 눌러 줄을 바꿔 입력한 경우 브라우저에 <br/> 태그로 바꿔 출력하기 위해 request 영역에
//	"\r\n"을 저장한다.
